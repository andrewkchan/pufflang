...
describe("Stage1 parser AST invalid input (missing EOF)", () => {
  test("returns root=-1 and empty arena when EOF token is missing", () => {
    const source = `
    ${astSrc}
    ${scannerSrc}
    ${parserSrc}
    def main() {
      var src = "def foo() { return 1; } 123"; // trailing junk; parse_simple should fail
