...
describe("Stage1 parser AST invalid input (missing right brace)", () => {
  test("returns root=-1 and empty arena when '}' is missing", () => {
    const source = `
    ${astSrc}
    ${scannerSrc}
    ${parserSrc}
    def main() {
      var src = "def foo() { return 1;"; // missing right brace
