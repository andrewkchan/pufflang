...
describe("Stage1 parser AST invalid input (missing var semicolon)", () => {
  test("returns root=-1 and empty arena", () => {
    const source = `
    ${astSrc}
    ${scannerSrc}
    ${parserSrc}
    def main() {
      var src = "def foo() { var x = 1 return 2; }"; // missing semicolon after var
