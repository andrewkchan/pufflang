...
describe("Stage1 parser AST invalid input (bad return token)", () => {
  test("returns root=-1 and empty arena when return token is not NUMBER/IDENT", () => {
    const source = `
    ${astSrc}
    ${scannerSrc}
    ${parserSrc}
    def main() {
      var src = "def foo() { return +; }";
