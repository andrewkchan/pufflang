...
describe("Stage1 expr operator count helper", () => {
  test("counts operators in simple expressions", () => {
    const source = `
${astSrc}
${scannerSrc}
${parserExprSrc}
...
def main() {
  var a = byte~(&"1+2*3"[0]);
  var b = byte~(&"a||b&&c?d:e"[0]);
  print_int(expr_operator_count(a, cstr_len(a)));
  __putchar__(32);
  print_int(expr_operator_count(b, cstr_len(b)));
  __putchar__(10);
}
`
